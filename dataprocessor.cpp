...
#define PROPORTIONDEFAULT       3.0//默认调节限制比例
...
/**
 * @brief DataProcessor::dataStartWork
 * 开启计时器
 */
void DataProcessor::dataStartWork()
{
    saveDataTimer->start(SaveDataTimeInterval*1000);//默认10s一次
    monitorTimer->start(MonitorTimeInterval*1000);//
}
#ifndef TESTMODE
void DataProcessor::connectammeter(QString ammetername)
{
...
{
    regulator->connectname(regulatorname);//
}
...
void DataProcessor::setTimeInterval(int interval, int timettype)
{
    switch (timettype) {
    case 1:
        GetDataTimeInterval = interval*60;
        break;
    case 2:
        SaveDataTimeInterval = interval*60;
        break;
    case 3:
        MonitorTimeInterval = interval*60;
        break;
    default:
        break;
    }
